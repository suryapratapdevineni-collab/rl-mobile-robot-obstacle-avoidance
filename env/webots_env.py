...
class WebotsEnv(gym.Env, Supervisor):
    ...
    def step(self, action):
        ...
        # Track dynamic historical milestone anchors
        if current_distance < self.best_distance:
            self.best_distance = current_distance

        # Terminate if the goal is achieved or if excessive crashing occurs
        exceeded_collision_limit = (self.total_collisions >= self.MAX_COLLISIONS_THRESHOLD)
        terminated = bool(goal_reached or exceeded_collision_limit)
        truncated = (self.step_count >= self.max_episode_steps)

        info = {
            "goal_reached": bool(goal_reached),
            "reached_goal": bool(goal_reached),
            "collision_count": int(self.total_collisions),
            "exceeded_collisions": bool(exceeded_collision_limit)
        }
